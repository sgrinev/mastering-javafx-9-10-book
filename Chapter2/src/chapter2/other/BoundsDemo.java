...
public class BoundsDemo extends Application {
    ...
    private VBox getTransformationControls() {
        ...
        HBox xyBox = new HBox();
        xyBox.setSpacing(5);
        xyBox.getChildren().addAll(new VBox(xLabel, xSlider),
                                   new VBox(yLabel, ySlider));

        HBox whBox = new HBox();
        whBox.setSpacing(5);
        whBox.getChildren().addAll(new VBox(widthLabel, widthSlider),
                                   new VBox(heightLabel, heightSlider));

        HBox colorBox = new HBox();
        colorBox.setSpacing(5);
        colorBox.getChildren().addAll(new VBox(strokeLabel, strokeSlider),
                                      new VBox(new Label("Stroke Color"),
                                                                    rectStrokeColorChoiceBox));

        HBox opacityBox = new HBox();
        opacityBox.setSpacing(5);
        opacityBox.getChildren().addAll(new VBox(opacityLabel, opacitySlider),
                                        new VBox(new Label("Fill Color"),
                                                                      rectFillColorChoiceBox));


        HBox translateBox = new HBox();
        translateBox.setSpacing(5);
        translateBox.getChildren().addAll(new VBox(translateXLabel, translateXSlider),
                                          new VBox(translateYLabel, translateYSlider));

        HBox rotateBox = new HBox();
        rotateBox.setSpacing(5);
        rotateBox.getChildren().addAll(new VBox(rotateLabel, rotateSlider));

        HBox scaleBox = new HBox();
        scaleBox.setSpacing(5);
        scaleBox.getChildren().addAll(new VBox(scaleXLabel, scaleXSlider),
                                      new VBox(scaleYLabel, scaleYSlider));

        HBox shearBox = new HBox();
        shearBox.setSpacing(5);
        shearBox.getChildren().addAll(new VBox(shearXLabel, shearXSlider),
                                      new VBox(shearYLabel, shearYSlider));
        ...
        resetAllButton.setOnAction((ActionEvent actionEvent) -> {
            resetAll();
        });
    ...
    private void attachEventHandlers() {
        ChangeListener<Number> cl = (o, oldV, newV) -> {
            relayout();
        };

        ChangeListener<String> clStr = (o, oldV, newV) -> {
            relayout();
        };
    ...
    private TitledPane getShowBoundsControls() {
        ChangeListener<Boolean> cl = (o, oldV, newV) -> {
            relayout();
        };

        ChangeListener<Boolean> cl2 = (o, oldV, newV) -> {
            animate();
        };
        ...
        effectGroup.selectedToggleProperty().addListener((o, oldV, newV) -> {
            relayout();
        });
